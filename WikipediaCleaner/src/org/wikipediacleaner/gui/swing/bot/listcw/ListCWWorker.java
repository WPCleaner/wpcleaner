...
public class ListCWWorker extends BasicWorker {
  ...
  /** Directory (or file with placeholder for error number) in which the output is written */
  private final File output;

  /** Page name (with placeholder for error number) in which the output is written */
   ...
   * @param output Directory (or file with placeholder for error number) in which the output is written.
   ...
   * @param window Window.
   * @param dumpFile File containing the dump to be analyzed.
   * @param pageName Page name (with placeholder for error number) in which the output is written.
   * @param selectedAlgorithms List of selected algorithms.
   * @param selectedNamespaces List of selected namespaces.
   * @param checkWiki True if last version of articles should be checked on wiki.
  ...
  void reportProgress() {
    ...
    buffer.append("\n");
    buffer.append("Pages processed: ").append(countAnalyzed);
    buffer.append(" / errors detected: ").append(countDetections);
    buffer.append(" Analysis: ").append(analysisTime.toString());
    for (AlgorithmInformation algorithm : selectedAlgorithms) {
      buffer.append(" Algorithm ")
          .append(algorithm.algorithm.getErrorNumberString())
          .append(": ")
          .append(algorithm.getTimeSpent() / 1000000000);
    }
  ...
  private void outputResult(CheckErrorAlgorithm algorithm, Collection<Detection> pages) {
    ...
    List<Detection> tmpPages = new ArrayList<>(
        pages.stream().filter(Objects::nonNull).sorted().toList());
  ...
  private boolean outputResultToPage(
    ...
    while (attemptCount < 10) {
      ...
      try {
        ...
        logCW.info("Trying with smaller list ({} bytes)", currentMaxSize);
      }
    }
  ...
  private void outputResultToFile(
    ...
    // Write the file
    logCW.info("Writing dump analysis results for error {} to file {}", algorithm.getErrorNumberString(), outputFile.getName());
