 ...
 */
public class CheckErrorAlgorithm23 extends CheckErrorAlgorithmBase {

  public CheckErrorAlgorithm23() {
  ...
  public boolean analyze(Page page, String contents, ArrayList<CheckErrorResult> errors) {
    ...
    while (startIndex < contents.length()) {
      startIndex = contents.indexOf("<nowiki>", startIndex);
      if (startIndex < 0) {
        startIndex = contents.length();
      } else {
        int endIndex = contents.indexOf("</nowiki>", startIndex);
        if (endIndex < 0) {
          if (errors == null) {
            return true;
          }
          result = true;
          endIndex = startIndex + "<nowiki>".length();
